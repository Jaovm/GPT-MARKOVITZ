...
def main():
    ...
    if ticker:
        ...
        if metrics:
            ...
            recommendation = ""
            try:
                if sector == "Technology" and float(metrics.get("P/S Ratio", 0)) < 5:
                    recommendation = "A ação pode estar barata."
                elif sector == "Energy" and float(metrics.get("EV/EBITDA", 10)) < 5:
                    recommendation = "A ação pode estar subvalorizada."
                elif sector == "Financial Services" and float(metrics.get("P/B Ratio", 1.5)) < 1:
                    recommendation = "A ação pode estar barata."
                elif sector == "Healthcare" and float(metrics.get("P/E Ratio", 20)) < 15:
                    recommendation = "A ação pode estar barata."
                else:
                    recommendation = "Não há sinais claros de subvalorização."
            except Exception as e:
                recommendation = "Erro na análise dos indicadores."
